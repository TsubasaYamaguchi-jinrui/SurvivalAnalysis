...
## ロジスティック回帰モデル  
離散時間モデルでは、ロジット変換[^foot3]を利用することでハザード関数$P(t)$を以下のようにあらわす。なお、$x_1$は非時間依存変数を、$x_2(t)$は時間依存変数を表す。また、$t$は勤続年数を表す。$b_1$と$b_2$は**偏回帰係数**と呼ぶ。ロジット変換を施すことで、$P(t)$が0から1の範囲しか取れなくても、右辺はどのような値も取ることが可能になる。    
...
Rでは以下のように行う。結果を見ると、モデル2の方が有意に適合度が高いことが分かる。    
```{r}
